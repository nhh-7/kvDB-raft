...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp)
{
    ...
    // 生成rpc方法调用的请求request和响应response参数,由于是rpc的请求，因此请求需要通过request来序列化 创建 Request 和 Response 对象，解析参数
    google::protobuf::Message *request = service->GetRequestPrototype(method).New();
    if (!request->ParseFromString(args_str)) //  把 args_str 解析到 request 对象里
    {
    ...
    // 业务方法执行完，框架会自动执行 done->Run()，也就是调用 SendRpcResponse(conn, response)
